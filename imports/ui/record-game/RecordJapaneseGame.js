...
Template.RecordJapaneseGame.onCreated( function() {
    ...
    if (localStorage.getItem("game_id") !== null) {
        ...
        Meteor.call('canRetrieveInProgressJapaneseGame', Session.get("game_id"), function (error, game) {
            ...
            Session.set("eastPlayerDealInTotal",  game.eastPlayerDealInTotal);
            Session.set("southPlayerDealInTotal", game.southPlayerDealInTotal);
            Session.set("westPlayerDealInTotal", game.westPlayerDealInTotal);
            Session.set("northPlayerDealInTotal", game.northPlayerDealInTotal);

            Session.set("eastPlayerDealInAfterRiichiTotal", game.eastPlayerDealInAfterRiichiTotal);
            Session.set("southPlayerDealInAfterRiichiTotal", game.southPlayerDealInAfterRiichiTotal);
            Session.set("westPlayerDealInAfterRiichiTotal", game.westPlayerDealInAfterRiichiTotal);
            Session.set("northPlayerDealInAfterRiichiTotal", game.northPlayerDealInAfterRiichiTotal);

            Session.set("eastPlayerSelfDrawTotal", game.eastPlayerSelfDrawTotal);
            Session.set("southPlayerSelfDrawTotal", game.southPlayerSelfDrawTotal);
            Session.set("westPlayerSelfDrawTotal", game.westPlayerSelfDrawTotal);
            Session.set("northPlayerSelfDrawTotal", game.northPlayerSelfDrawTotal);

            Session.set("eastPlayerRiichiEV", game.eastPlayerRiichiEV);
            Session.set("southPlayerRiichiEV", game.southPlayerRiichiEV);
            Session.set("westPlayerRiichiEV", game.westPlayerRiichiEV);
            Session.set("northPlayerRiichiEV", game.northPlayerRiichiEV);
        });
    }
});
...
Template.RecordJapaneseGame.events({
    ...
    'click .submit_names_button'(event, template) {
        ...
        let game = {
            ...
            eastPlayerDealInTotal: 0,
            southPlayerDealInTotal: 0,
            westPlayerDealInTotal: 0,
            northPlayerDealInTotal: 0,
            eastPlayerDealInAfterRiichiTotal: 0,
            southPlayerDealInAfterRiichiTotal: 0,
            westPlayerDealInAfterRiichiTotal: 0,
            northPlayerDealInAfterRiichiTotal: 0,
            eastPlayerSelfDrawTotal: 0,
            southPlayerSelfDrawTotal: 0,
            westPlayerSelfDrawTotal: 0,
            northPlayerSelfDrawTotal: 0,
            eastPlayerRiichiEV: 0,
            southPlayerRiichiEV: 0,
            westPlayerRiichiEV: 0,
            northPlayerRiichiEV: 0
        };
    ...
    'click .submit_hand_button'(event, template) {
        ...
        if ( !$( event.target ).hasClass( "disabled")) {
            ...
            let game = {
                ...
                eastPlayerDealInTotal: Session.get("eastPlayerDealInTotal"),
                southPlayerDealInTotal: Session.get("southPlayerDealInTotal"),
                westPlayerDealInTotal: Session.get("westPlayerDealInTotal"),
                northPlayerDealInTotal: Session.get("northPlayerDealInTotal"),
                eastPlayerDealInAfterRiichiTotal: Session.get("eastPlayerDealInAfterRiichiTotal"),
                southPlayerDealInAfterRiichiTotal: Session.get("southPlayerDealInAfterRiichiTotal"),
                westPlayerDealInAfterRiichiTotal: Session.get("westPlayerDealInAfterRiichiTotal"),
                northPlayerDealInAfterRiichiTotal: Session.get("northPlayerDealInAfterRiichiTotal"),
                eastPlayerSelfDrawTotal: Session.get("eastPlayerSelfDrawTotal"),
                southPlayerSelfDrawTotal: Session.get("southPlayerSelfDrawTotal"),
                westPlayerSelfDrawTotal: Session.get("westPlayerSelfDrawTotal"),
                northPlayerSelfDrawTotal: Session.get("northPlayerSelfDrawTotal"),
                eastPlayerRiichiEV: Session.get("eastPlayerRiichiEV"),
                southPlayerRiichiEV: Session.get("southPlayerRiichiEV"),
                westPlayerRiichiEV: Session.get("westPlayerRiichiEV"),
...
function save_game_to_database(hands_array) {
    ...
    if (Session.get("upperJapaneseGame")) {
        // Initialise ELO calculator to update player ELO
        let upper_jpn_elo_calculator = new EloCalculator(Constants.ELO_CALCULATOR_N,
            Constants.ELO_CALCULATOR_EXP,
            Constants.JPN_SCORE_ADJUSTMENT,
            game,
        ...
        let game3 = {
            positions: Constants.WINDS.map((wind) => ({ wind, score: Session.get(wind + "_score") })).sort((a, b) => b.score - a.score),
            hands_array_length: hands_array.length,
            east_elo_delta: upper_jpn_elo_calculator.eloChange(east_player),
            south_elo_delta: upper_jpn_elo_calculator.eloChange(south_player),
            west_elo_delta: upper_jpn_elo_calculator.eloChange(west_player),
            north_elo_delta: upper_jpn_elo_calculator.eloChange(north_player),
            east_player: east_player,
            south_player: south_player,
            west_player: west_player,
            north_player: north_player,
